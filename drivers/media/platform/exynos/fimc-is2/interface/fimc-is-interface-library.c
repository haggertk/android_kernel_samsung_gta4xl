...
static void fimc_is_get_fd_data_vra(u32 instance,
...
static void fimc_is_get_fd_data_nfd(u32 instance,
	struct fd_info *face_data,
	struct fd_rectangle *fd_in_size)
{
	struct fimc_is_lib_support *lib = &gPtr_lib_support;
	...
	ischain = &core->ischain[instance];
	is_region = ischain->is_region;
	nfd_data = (struct nfd_info *)&is_region->fd_info;

	spin_lock(&is_region->fd_info_slock);
	fd_in_size->width = nfd_data->width;
	fd_in_size->height = nfd_data->height;

	face_data->face_num = nfd_data->face_num;
	face_data->frame_count = nfd_data->frame_count;

	for (i = 0; i < face_data->face_num; i++) {
		face_data->face[i].faceId = nfd_data->id[i];
		face_data->face[i].score = nfd_data->score[i];
		face_data->face[i].rotation = nfd_data->rot[i];
		face_data->face[i].face_area.span.width = nfd_data->rect[i].width;
		face_data->face[i].face_area.span.height = nfd_data->rect[i].height;
		face_data->face[i].face_area.top_left.x = nfd_data->rect[i].offset_x;
		face_data->face[i].face_area.top_left.y = nfd_data->rect[i].offset_y;
	}
	spin_unlock(&is_region->fd_info_slock);
	...
	struct fd_rectangle *fd_in_size)
{
	...
	if (ischain->use_vra_nfd == USE_NFD)
		fimc_is_get_fd_data_nfd(instance, face_data, fd_in_size);
	else if (ischain->use_vra_nfd == USE_VRA)
		fimc_is_get_fd_data_vra(instance, face_data, fd_in_size);
